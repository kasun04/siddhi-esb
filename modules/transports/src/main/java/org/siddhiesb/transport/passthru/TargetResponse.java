...
public class TargetResponse {
    ...
    public void start(NHttpClientConnection conn) {
        org.siddhiesb.transport.passthru.TargetContext.updateState(conn, org.siddhiesb.transport.passthru.ProtocolState.RESPONSE_HEAD);
        
        if (expectResponseBody) {
            pipe
                = new org.siddhiesb.transport.passthru.Pipe(conn, targetConfiguration.getBufferFactory().getBuffer(), "target", targetConfiguration);
    ...
    public int read(NHttpClientConnection conn, ContentDecoder decoder) throws IOException {
        ...
        if (decoder.isCompleted()) {
            org.siddhiesb.transport.passthru.TargetContext.updateState(conn, org.siddhiesb.transport.passthru.ProtocolState.RESPONSE_DONE);
